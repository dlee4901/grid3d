...
{
    private static readonly Dictionary<(Type valueType, string name), Delegate> _accessors = new();
    ...
    public static Func<TTarget, TValue> Get<TValue>(string name)
    {
        if (!_accessors.TryGetValue((typeof(TValue), name), out var del))
            throw new InvalidOperationException($"Accessor '{name}' of type '{typeof(TValue).Name}' not registered");
