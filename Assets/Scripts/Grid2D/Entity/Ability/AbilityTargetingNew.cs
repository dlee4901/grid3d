...
public abstract class AbilityTargetingNew
{
...
public class PositionTargeting : AbilityTargetingNew
{
    public List<GridSelection> SelectableArea { get; set; }
    public DirectionTargeting Chain { get; set; } = null;
    
    public HashSet<(int, int)> GetSelectablePositions(QueryContext ctx)
    {
        var selectablePositions = new HashSet<(int, int)>();
        foreach (var selection in SelectableArea) selectablePositions.UnionWith(selection.GetPositions(ctx));
        return selectablePositions;
    }
    
    public HashSet<GridStep> GetSelectableSteps(QueryContext ctx)
    {
        var selectablePositions = new HashSet<GridStep>();
        foreach (var selection in SelectableArea) selectablePositions.UnionWith(selection.GetSteps(ctx));
        return selectablePositions;
    }
...
public class DirectionTargeting : AbilityTargetingNew
{
